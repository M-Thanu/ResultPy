...
def app():
    ...
    def extract_results(pdf_bytes, module_name):
        """Extract IndexNumber and Grade from your PDF format."""
        doc = fitz.open(stream=pdf_bytes, filetype="pdf")
        pdf_text = ""
        for page in doc:
            pdf_text += page.get_text("text")

        # Regex: IndexNumber | Grade (handles | and spaces)
        ...
        df = pd.DataFrame(matches, columns=["IndexNumber", module_name])
        return df

    # -----------------------------
    # Main App
    def main():
        st.title("📊 GPA Calculator - Module Results Merger")
